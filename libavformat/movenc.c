...
static int getVFrameCount(AVStream* st)
{
	...
	av_log(NULL, AV_LOG_DEBUG, "input dir %x\n", dirEnd);
	if(dirEnd)
		av_strlcpy(inputDir, inputPath, dirEnd - inputPath + 2);
	else
		strcpy(inputDir, ".");
...
static int setupTrackInfo(AVFormatContext *s)
{
	...
	for(int i = 0; i < mov->nb_streams; i++)
	{
        ...
        if (track->enc->codec_type == AVMEDIA_TYPE_VIDEO)
        {
    		...
    		av_log(s, AV_LOG_INFO, "setupTrackInfo video branch duration %d entry %d\n", track->total_duration, track->entry);
	...
	retval = 1;
error:
	av_log(s, AV_LOG_INFO, "setupTrackInfo %s exit retval: %d\n", s->filename, retval);
	return retval;
}
...
static int mov_write_stco_tag(AVIOContext *pb, MOVMuxContext *mov,
                              MOVTrack *track)
{
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_stco_tag enter %d total duration %lx\n",
			track->enc->codec_type, track->total_duration);
    int i;
    int mode64 = 0; //use 32 bit size variant if possible
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size */
    if(globalFormat->linked_content) {
    ...
    if(!globalFormat->linked_content || track->enc->codec_type != AVMEDIA_TYPE_AUDIO) {
    	if(globalFormat->linked_content) {
			mode64 = 1;
			avio_wtag(pb, "co64");
    	}
	...
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_stco_tag exit %d\n", track->enc->codec_type);
    return updateSize(pb, pos);
}
/* Sample size atom */
static int mov_write_stsz_tag(AVIOContext *pb, MOVTrack *track)
{
    av_log(NULL, AV_LOG_DEBUG, "mov_write_stsz_tag enter %x\n", track->enc->codec_type);
    ...
    if(!globalFormat->linked_content || track->enc->codec_type == AVMEDIA_TYPE_VIDEO) {
		for (i=0; i<track->entry; i++) {
    ...
    if(globalFormat->linked_content && track->enc->codec_type != AVMEDIA_TYPE_VIDEO) { // audio sizes are fixed.
		  ...
		  if(track->enc->codec_type == AVMEDIA_TYPE_DATA) {
		   ...
		   entries = track->entry;
			  ...
			  avio_wb32(pb, sSize); // sample size
			  avio_wb32(pb, entries); // sample count
		  av_log(globalFormat, AV_LOG_DEBUG, "sample size %d # samples %d\n", sSize, entries);
	}
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_stsz_tag exit\n");
    return updateSize(pb, pos);
}
...
{
	av_log(NULL, AV_LOG_DEBUG, "mov_write_stsc_tag enter media %d chunkCount %d entry %d\n",
			track->enc->codec_type,
			track->chunkCount, track->entry);
    ...
    av_log(NULL, AV_LOG_DEBUG, "mov_write_stsc_tag exit\n");
    return updateSize(pb, pos);
}
...
{
	av_log(NULL, AV_LOG_DEBUG, "codec: %x\n", track->enc->codec_id);
    int64_t pos = avio_tell(pb);
    int version = 0;
    ...
    if (!tag || (track->enc->strict_std_compliance >= FF_COMPLIANCE_NORMAL &&
        ...
        if (track->enc->codec_id == CODEC_ID_DVVIDEO)
            ...
            if (!tag) { // if no mac fcc found, try with Microsoft tags
                ...
                if (tag)
                    av_log(s, AV_LOG_DEBUG, "Warning, using MS style video codec tag, "
            ...
            if (!tag) { // if no mac fcc found, try with Microsoft tags
                ...
                if (ms_tag) {
                    ...
                    av_log(s, AV_LOG_DEBUG, "Warning, using MS style audio codec tag, "
                           "the file may be unplayable!\n");
                }
            }
...
{
	av_log(NULL, AV_LOG_DEBUG, "mov_write_clap_tag width %d height %d\n", track->enc->width, track->height);
    avio_wb32(pb, 40);
    avio_wtag(pb, "clap");
    if(globalFormat->linked_content)
    	avio_wb32(pb, 0x4e0);
...
{
	av_log(s, AV_LOG_DEBUG, "mov_write_colr_tag %d height %d width %d\n",
			track->enc->color_primaries,
			track->enc->height, track->enc->width);
...
static int mov_write_video_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track)
{
    ...
    av_log(s, AV_LOG_DEBUG, "mov_write_video_tag %s %d\n",
    		(char *)&track->tag,
    		track->enc->codec_id);
...
static const char* replacechar(char* string, char src, char dest)
{
	char* new_string = av_malloc(strlen(string) + 2);
	int i;
...
/**
 * The original version of dref function
 */
static int mov_write_origdref_tag(AVIOContext *pb)
{
...
static int mov_write_dref_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if(!globalFormat->linked_content)
    	return mov_write_origdref_tag(pb);
    ...
    av_log(globalFormat, AV_LOG_DEBUG, "mov_write_dref_tag exit\n");
...
static int mov_write_minf_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track)
{
    ...
    if(globalFormat->linked_content)
    	...
    	mov_write_dinf_tag(pb, NULL);

    mov_write_stbl_tag(s, pb, track);
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_minf_tag exit %d\n", track->enc->codec_type);
    return updateSize(pb, pos);
}
...
static int mov_write_mdhd_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if(globalFormat->linked_content) {
		int64_t duration = mvhd_duration*track->timescale/MOV_TIMESCALE;
		  av_log(globalFormat, AV_LOG_DEBUG, "mov_write_mdhd_tag duration %ld\n", duration);
		if (version == 1)
...
{
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_mdia_tag enter %d\n", track->enc->codec_type);
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size */
    avio_wtag(pb, "mdia");
	...
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_mdia_tag exit %d\n", track->enc->codec_type);
    return updateSize(pb, pos);
}

static int mov_write_tkhd_tag(AVIOContext *pb, MOVTrack *track, AVStream *st)
{
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_tkhd_tag exit %d\n", track->enc->codec_type);
...
static int mov_write_trak_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track, AVStream *st)
{
	if(globalFormat->linked_content) {
		...
		track->enc->sample_aspect_ratio.num = 1;
	}
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size */
    avio_wtag(pb, "trak");
...
{
	av_log(globalFormat, AV_LOG_INFO, "mov_write_mvhd_tag enter\n");
    int maxTrackID = 1, i;
	...
	av_log(globalFormat, AV_LOG_DEBUG, "mov_write_mvhd_tag exit\n");
    return 0x6c;
}
...
{
	av_log(globalFormat, AV_LOG_DEBUG, "build_chunks enter: media %d entry %d\n",
			trk->enc->codec_type, trk->entry);
    ...
    av_log(globalFormat, AV_LOG_DEBUG, "build_chunks exit\n");
}
                              ...
                              AVFormatContext *s)
{
    int i, j;
    int64_t pos = avio_tell(pb);
    av_log(s, AV_LOG_INFO, "mov_write_moov_tag enter %d linked_content %d\n", mov->nb_streams, s->linked_content);
    avio_wb32(pb, 0); /* size placeholder*/
    avio_wtag(pb, "moov");
    ...
    for (i=0; i<mov->nb_streams; i++) {
        ...
        av_log(s, AV_LOG_DEBUG, "edit_duration %ld\n", track->edit_duration);
    ...
    for (i=0; i<mov->nb_streams; i++) {
    	av_log(s, AV_LOG_DEBUG, "nb_streams %d %d\n", mov->nb_streams, mov->tracks[i].entry);
    ...
    av_log(s, AV_LOG_DEBUG, "mov_write_moov_tag exit\n");
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    av_log(s, AV_LOG_DEBUG, "mov_write_packet %d %d %d %d\n",
			enc->codec_id, size, pkt->stream_index, CODEC_ID_MPEG2VIDEO);
...
static int mov_create_timecode_track(AVFormatContext *s, int tracknum)
{
    ...
    av_log(globalFormat, AV_LOG_DEBUG, "mov_create_timecode_track enter timecode %s tracknum %d\n", mov->timecode, tracknum);
    ...
    av_free_packet(&pkt);
    av_log(globalFormat, AV_LOG_DEBUG, "mov_create_timecode_track exit\n");
    return 0;
}
...
{
	av_log(s, AV_LOG_DEBUG, "mov_compute_moov_size\n");
    MOVMuxContext *mov = s->priv_data;
    AVIOContext *pb;
    uint8_t *buf;
    int i, size;
    ...
    av_log(s, AV_LOG_DEBUG, "moov_compute_size close dyn buf %d\n", size);
    av_free(buf);

    for (i = 0; i < mov->nb_streams; i++) {
        MOVTrack *track = &mov->tracks[i];
        av_log(s, AV_LOG_DEBUG, "entry %d\n", track->entry);
        if((track->entry > 0 && !s->linked_content) ||
        	(track->entry > 0 &&
        		s->linked_content && track->enc->codec_type != AVMEDIA_TYPE_AUDIO)) {
        	if (track->cluster[track->entry-1].pos < UINT32_MAX &&
                track->cluster[track->entry-1].pos +
                size - mov->free_size > UINT32_MAX) {
                size += track->entry*4;
                av_log(s, AV_LOG_DEBUG, "moov_compute_size track entry %d\n", size);
            }
        }
    }
    av_log(s, AV_LOG_DEBUG, "moov_compute_size %d\n", size);
    return size;
}
...
static int mov_populate_video_index(MOVMuxContext *mov, char* inputFile)
{
	...
	for(int i = 0; i < mov->nb_streams; i++) {
	    ...
	    track->cluster = av_malloc(track->entry * sizeof(MOVIentry));
	    if (!track->cluster)
	        return -1;

	    // populate cluster array
	    av_log(NULL, AV_LOG_DEBUG, "track->entry %d\n", track->entry);
		for(int j = 0; j < track->entry; j++) {
			if(fread(buffer, 1, 16, fp) < 16)
				return -1;

			uint64_t pos = *(uint64_t *)&buffer[0]; //reverse8(buffer);
			uint32_t size = *(uint32_t *)&buffer[8];//reverse4(buffer + 8);
			track->cluster[j].pos = pos;
			track->cluster[j].size = size;
		    track->cluster[j].samplesInChunk = 1;
...
{
	av_log(s, AV_LOG_DEBUG, "mov_overwrite_file enter\n");
    MOVMuxContext *mov = s->priv_data;
